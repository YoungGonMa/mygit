...
<#
	...
	PassThru 로 호출한 후, WaitForExit으로 기다린다.
	PassThru와 함께 적용할 수 없다.
...
#>
	...
	(
		...
		[parameter(Mandatory=$false,ParameterSetName="WaitForExit")]
		[switch] $WaitForExit
	)
	...
	PROCESS
	{
		...
		if (!(Test-Path Variable:Private:WaitForExit)) { $WaitForExit = $false }
		...
		try
		{
			...
			if ($PromptForInputBeforeClosing -and (!$PassThru -or !$WaitForExit) `
			...
			elseif ($WaitForExit)
			{
				$performBuildScriptBlock =
				{
					...
					if ($ShowBuildOutputInCurrentWindow)
					{
						$proc = Start-Process cmd.exe -ArgumentList $cmdArgumentsToRunMsBuild -NoNewWindow -PassThru
					}
					else
					{
						$proc = Start-Process cmd.exe -ArgumentList $cmdArgumentsToRunMsBuild -WindowStyle $windowStyleOfNewWindow -PassThru
					}
					$handle = $proc.Handle # cache proc.Handle - ExitCode를 받으려면 필요하다.
					$proc.WaitForExit()
					$result.MsBuildProcess = $proc
				}

...
$script:buildConfigurations = @(
    @{ Module = "LibCommon";        Platforms = @("Win32", "x64") },
    @{ Module = "MsgpackRpc";       Platforms = @("Win32", "x64") },
    @{ Module = "UtilityIF";        Platforms = @("Win32", "x64") },
    @{ Module = "ECMIF";            Platforms = @("Win32", "x64") },
...
function check() {
    ...
    If ($script:targetModules.Contains("DestinyECMIF64.dll")) {
        ...
        $script:buildConfigurations += @{ Module = "ECMIF"; Platforms = @("x64") }
    }
    ...
    If ($script:targetModules.Contains("DestinyUtilityIF64.dll")) {
        $script:buildConfigurations += @{ Module = "LibCommon"; Platforms = @("x64") }
        $script:buildConfigurations += @{ Module = "UtilityIF"; Platforms = @("x64") }
    }

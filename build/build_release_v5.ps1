...
$script:buildUpdateStatusBoard = $false
$script:buildUpdater = $false
$script:buildUpdateDownloader = $false
$script:buildSmartUpdaterX = $false
$script:buildAssistService = $false
$script:buildRes = $false
$script:buildLogCollector = $false
$script:buildProgramCheck = $false
...
if ($script:testMode) {
    ...
    $script:targetModuleString1 = "DestinyProgramCheck.exe" # DestinyECMAgent.exe,DestinyMsgMgr.exe,DestinyECMIF.dll,DestinyECMIF64.dll,DestinyUtilityIF.dll,DestinyUtilityIF64.dll,DestinyLocalSyncIF.dll,DestinyLocalSyncIF64.dll,LogCollector.exe,DestinyProgramCheck.exe
...
function build() {
	...
	If ($script:buildECMIF64) {
        buildSln "ECMIF.sln" $false
    }
	If ($script:buildLogCollector) {
        buildSln "LogCollector.sln" $true
    }
	If ($script:buildProgramCheck) {
        buildSln "ProgramCheck.sln" $true
    }
    ...
    If ($script:buildUpdater) {
        buildSln "Updater.sln" $true
    }
    If ($script:buildUpdateDownloader) {
        buildSln "UpdateDownloader.sln" $true
    }
    If ($script:buildUpdateStatusBoard) {
        buildSln "UpdateStatusBoard.sln" $true
    }
...
function check() {
	...
	If ($script:targetModules.Contains("LogCollector.exe")) {
        $script:buildLogCollector = $true
    }
	
	If ($script:targetModules.Contains("DestinyProgramCheck.exe")) {
		$script:buildLibCommon32 = $true
        $script:buildProgramCheck = $true
    }
...
function cleanAll {
	...
	clean "LogCollector.sln" $true
	clean "ProgramCheck.sln" $true

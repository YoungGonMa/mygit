...
# version5 - ecm,cloudium,startia,ktbizez
...
if ($Env:COMMIT_TARGET_SM) {
	if ($_filesSU -eq "") {
		$_filesSU = $Env:COMMIT_TARGET_SM.Split(",")
	} else {
		$_filesSU += $Env:COMMIT_TARGET_SM.Split(",")
	}
}
...
function copyBin($product)
{
    ...
    foreach ($item in $_files)
    {
        ...
        if ($item -eq "DestinyECMIF.dll")
        {
            $destPath = $destPath0 + "DestinyCheckin\"
			...
			$destPath = $destPath0 + "DestinyDriveVDisk\dLooko\"
			Write-Host "destPath: " $destPath
			Write-Host "target: " $binPath $item
            $binPath + $item | Copy-Item -Destination $destPath -Force			
         }
		 elseif ($item -eq "DestinyECMIF64.dll")
		 {
			$destPath = $destPath0 + "DestinyDriveVDisk\dLooko\"
			Write-Host "destPath: " $destPath
			Write-Host "target: " $binPath $item
			...
			#ARM 복사
			# $binARMPath = $binPath + "ARM\"  # ARM 폴더 경로 추가
			# $destPath = $destPath0 + "DestinyDriveVDisk\dLooko\ARM\"
			# Write-Host "destPath: " $destPath
			# Write-Host "target: " $binARMPath $item
            # $binARMPath + $item | Copy-Item -Destination $destPath -Force
			
		 }
		 elseif ($item -eq "DestinyUtilityIF.dll")
		 {
		 ...
		 elseif ($item -eq "DestinyUtilityIF64.dll")
		 {
            $destPath = $destPath0 + "DestinyECMAgent\"
			Write-Host "destPath: " $destPath
			Write-Host "target: " $binPath $item
			...
			#ARM 복사
			# $binARMPath = $binPath + "ARM\"  # ARM 폴더 경로 추가			
            # $destPath = $destPath0 + "DestinyECMAgent\ARM\"
			# Write-Host "destPath: " $destPath
			# Write-Host "target: " $binARMPath $item
            # $binARMPath + $item | Copy-Item -Destination $destPath -Force						
		 }		 
         ...
         else # DestinyECMAgent.exe DestinyRes.dll
         {
            $destPath = $destPath0 + "DestinyECMAgent\"
			Write-Host "destPath: " $destPath
			Write-Host "target: " $binPath $item
...
function commit_internal($commitMessage, [ref]$items)
{
    ...
    foreach ($item in $_files)
    {
         if ($item -eq "DestinyECMIF.dll")
         {
            $argumentString += "solution\" + "DestinyCheckin\" + $item +"|"
            $argumentString += "solution\" + "DestinyDrive\" + $item +"|"
			$argumentString += "solution\" + "DestinyDriveVDisk\dLooko\" + $item +"|"
         }
		 elseif ($item -eq "DestinyECMIF64.dll")
		 {
			$argumentString += "solution\" + "DestinyDriveVDisk\dLooko\" + $item +"|"
			# $argumentString += "solution\" + "DestinyDriveVDisk\dLooko\ARM\" + $item +"|"
		 }
		 elseif ($item -eq "DestinyUtilityIF.dll")
		 {
		 ...
		 {
			$argumentString += "solution\" + "DestinyECMAgent\" + $item +"|"
			# $argumentString += "solution\" + "DestinyECMAgent\ARM\" + $item +"|"
		 }		 
         elseif ($item -eq "DestinyLocalSyncIF.dll" -Or $item -eq "DestinyLocalSyncIF64.dll")
         {
         ...
         elseif ($item -ne "") # DestinyECMAgent.exe DestinyRes.dll
         {
            $argumentString += "solution\" + "DestinyECMAgent\" + $item +"|"
         }
    }
    ...
    Write-Host "commit_internal : " $arguments
}
